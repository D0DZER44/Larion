...
function resolveManualActivityCode(value = "") {
  const normalized = normalizeText(value);
  if (!normalized) return "";
  return mapHumanActivityToCode(value);
}
...
export function mapInspectionItemsToAnswers(items = []) {
  return items.map((item, index) => {
    ...
    return {
      ...
      regraId: item.regraId || item.ruleId || item.id || `ans-${index}`,
...
export function mapTargetInspectionToMotor(inspection = {}) {
  const checklistName =
    ...
    inspection.nome ||
    "Inspeção";
  const resolvedActivityCode =
    inspection.activityId ||
    inspection.activityCode ||
    inspection.atividades?.[0] ||
    resolveManualActivityCode(inspection.tipoInspecao || checklistName);
  return {
    id: inspection.id,
    ...
    atividades: resolvedActivityCode ? [resolvedActivityCode] : [],
...
function resolveSectorIdFromState(state = {}, inspectionInput = {}, inspection = {}) {
  if (inspectionInput.sectorId) return inspectionInput.sectorId;
  const normalizedSector = normalizeText(
    inspectionInput.setor || inspectionInput.ondeUsar || inspection.setor,
  );
  const matchedSector = (state.sectors || []).find((sector) => {
    return (
      normalizeText(sector?.id) === normalizedSector ||
      normalizeText(sector?.name) === normalizedSector
    );
  });
  return matchedSector?.id || "";
}

function mapNonConformityToTarget(item = {}, inspectionInput = {}, inspection = {}, state = {}) {
  return {
    ...item,
    inspectionId: inspection.id,
    inspecaoId: inspection.id,
    sectorId: resolveSectorIdFromState(state, inspectionInput, inspection),
    activityId:
      inspectionInput.activityId ||
      inspection.activityId ||
      inspection.atividades?.[0] ||
      "",
    nrCode: item.nr || "",
...
export function synchronizeInspectionWithMotor(inspectionInput = {}, state = {}, options = {}) {
  ...
  const packageContext =
    inspectionInput.pacote ||
    inspectionInput.package ||
    inspectionInput.pacotesAtivos?.[0] ||
    "Base SST";
  const generatedNonConformities = generateNonConformitiesFromInspection(
    inspection,
    answers,
    {
      activityType: inspectionInput.activityId || inspection.atividades?.[0] || undefined,
      sector: inspection.setor,
      pacote: packageContext,
      responsavel: inspection.responsavel,
      rulePackages: state.rulePackages || [],
    },
  );
  ...
  const nonConformities = [...generatedNonConformities, ...fallbackNonConformities].map((item) =>
    mapNonConformityToTarget(item, inspectionInput, inspection, state),
  );
  const shouldGenerateOperationalEntities = options.persistMode !== "nonConformitiesOnly";
  const generatedRisks = shouldGenerateOperationalEntities
    ? generateRisksFromNonConformities(nonConformities, {
        responsavel: inspection.responsavel,
        recurrence: 0,
      })
    : [];
  const generatedActions = shouldGenerateOperationalEntities
    ? generateActionsFromRisks(generatedRisks, {
        responsavel: inspection.responsavel,
  ...
  const alerts = shouldGenerateOperationalEntities ? [
    ...risks
  ...
  const logs = [
    {
      ...
      description:
        options.persistMode === "nonConformitiesOnly"
          ? `Motor processou ${nonConformities.length} não conformidades para a inspeção ${inspection.titulo}.`
          : `Motor processou ${nonConformities.length} não conformidades, ${risks.length} riscos e ${actions.length} ações para a inspeção ${inspection.titulo}.`,
      ...
      metadata: {
        ...
        persistMode: options.persistMode || "full",
      },
    },
  ];
  ...
  return {
    inspection: {
      ...
      pacote:
        inspectionInput.pacote ||
        findPackageOfNR(risks[0]?.nr || nonConformities[0]?.nr || "NR-01")?.id ||
        packageContext,
      nonConformities: nonConformities.length,
