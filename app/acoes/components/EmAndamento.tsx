  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-[var(--bg-secondary)] border border-[var(--border)] p-5 rounded-[12px] flex flex-col justify-between">
           <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="text-[14px] font-medium text-[var(--text-secondary)]">Em andamento</h3>
           </div>
        ...
        <div className="bg-[#1a1315] border border-red-500/20 p-5 rounded-[12px] flex flex-col justify-between">
           <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="text-[14px] font-medium text-[var(--text-secondary)]">Críticas em execução</h3>
           </div>
           <div className="mt-2 text-[var(--text-primary)] font-bold text-[32px] leading-tight flex items-baseline justify-between w-full">
              {stats.criticas}
        ...
        <div className="bg-[#1a1713] border border-orange-500/20 p-5 rounded-[12px] flex flex-col justify-between">
           <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="text-[14px] font-medium text-[var(--text-secondary)]">Prazo médio restante</h3>
           </div>
           <div className="mt-2 text-[var(--text-primary)] font-bold text-[32px] leading-tight flex items-baseline justify-between w-full">
        ...
        <div className="bg-[#131a16] border border-emerald-500/20 p-5 rounded-[12px] flex flex-col justify-between">
           <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="text-[14px] font-medium text-[var(--text-secondary)]">Progresso médio</h3>
           </div>
           <div className="mt-2 text-[var(--text-primary)] font-bold text-[32px] leading-tight flex items-baseline justify-between w-full">
      ...
      <div className="flex flex-wrap items-center gap-6 bg-[var(--bg-secondary)] border border-[var(--border)] p-4 rounded-[12px]">
               ...
               return (
                  <button 
                     ...
                     className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${
                        isActive ? 'bg-[var(--bg-active-group)] text-[var(--text-primary)] border-[var(--border)]' : 'bg-transparent text-[var(--text-muted)] border-transparent hover:bg-[var(--bg-active-group)] hover:text-[var(--text-secondary)]'
                     }`}
                  >
                     {p !== 'Todos' && (
         ...
         <div className="w-px h-6 bg-[var(--bg-active-group)] hidden md:block"></div>
         <div className="flex items-center gap-2">
            <button key="TodosSetor" onClick={() => setFilterSetor('Todos')} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${filterSetor === 'Todos' ? 'bg-[var(--bg-active-group)] text-[var(--text-primary)] border-[var(--border)]' : 'bg-transparent text-[var(--text-muted)] border-transparent hover:bg-[var(--bg-active-group)] hover:text-[var(--text-secondary)]'}`}>
               Todos
            </button>
            {['Obras', 'Manutenção', 'Produção', 'Logística'].map(s => (
               <button 
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${filterSetor === s ? 'bg-[var(--bg-active-group)] text-[var(--text-primary)] border-[var(--border)]' : 'bg-transparent text-[var(--text-muted)] border-transparent hover:bg-[var(--bg-active-group)] hover:text-[var(--text-secondary)]'}`}
               >
                  {s}
               </button>
            ))}
        ...
        <table className="w-full text-left border-collapse min-w-[1000px]">
           <thead className="bg-[#0c1018]">
              <tr>
                 ...
                 <th className="px-5 py-3.5 text-xs font-medium text-[var(--text-muted)]">Setor</th>
                 <th className="px-5 py-3.5 text-xs font-medium text-[var(--text-muted)]">Progresso</th>
           ...
           <tbody className="divide-y divide-[var(--border)] text-[13px]">
              {list.map(acao => {
                 ...
                 return (
                    <tr key={acao.id} onClick={() => onOpen(acao)} className="hover:bg-[var(--bg-active-group)] transition-colors cursor-pointer group">
                       ...
                       <td className="px-5 py-4 w-32">
                          <span className="text-[var(--text-secondary)]">{acao.setor}</span>
                       </td>
                       <td className="px-5 py-4 w-44">
              ...
              {list.length === 0 && (
                 <tr>
                    <td colSpan={7} className="px-5 py-12 text-center text-sm text-[var(--text-muted)]">Nenhuma ação encontrada com os filtros selecionados.</td>
                 </tr>
              )}
           </tbody>
        </table>
