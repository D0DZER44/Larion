      ...
      >
        <div className="w-[440px] h-full flex flex-col overflow-hidden">
          ...
          <div className="flex items-center justify-between p-6 border-b border-[var(--border)] shrink-0">
            <div>
              <h2 className="text-[11px] font-medium text-[var(--text-muted)] uppercase tracking-widest mb-1.5">Detalhes da ação</h2>
            </div>
            <button onClick={onClose} className="p-1.5 bg-[var(--bg-active-group)] hover:bg-[var(--bg-active-group)] rounded-md text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors shrink-0 border border-[var(--border)]">
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-8 no-scrollbar">
            ...
            <div className="space-y-4">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1.5 flex-1 pr-2">
                   <h1 className="text-[18px] font-medium text-[var(--text-primary)] leading-snug">{acao.titulo}</h1>
                   {(isAndamento || isConcluida) && <div className="text-[12px] text-[var(--text-muted)]">ID: AC-{acao.id || '1011'}</div>}
                </div>
              ...
              {!isConcluida && !isAndamento && (
                <div className="space-y-2">
                   <h3 className="text-[12px] font-medium text-[var(--text-primary)]">Descrição</h3>
               ...
               <div className="space-y-2 pb-2">
                  <div className="bg-[var(--bg-secondary)]/50 p-4 rounded-xl border border-blue-500/20 space-y-3">
                     ...
                     <div className="space-y-1.5 text-[12px] text-[var(--text-secondary)]">
                        {(acao.nrRelacionada || (acao as any).nr) && <p><span className="font-bold text-[var(--text-muted)]">NR Relacionada:</span> {acao.nrRelacionada || (acao as any).nr}</p>}
                        {acao.regraTitulo && <p><span className="font-bold text-[var(--text-muted)]">Regra:</span> {acao.regraTitulo}</p>}
                        {acao.regraId && <p><span className="font-bold text-[var(--text-muted)]">Regra ID:</span> {acao.regraId}</p>}
                        ...
                        {acao.riscoVinculado && <p><span className="font-bold text-[var(--text-muted)]">Risco vinculado:</span> {acao.riscoVinculado || acao.riscoId}</p>}
                        {acao.inspecaoId && <p><span className="font-bold text-[var(--text-muted)]">Inspeção:</span> {acao.inspecaoId.substring(0,8).toUpperCase()}</p>}
                        {acao.perguntaOrigem && <p><span className="font-bold text-[var(--text-muted)]">Pergunta:</span> {acao.perguntaOrigem}</p>}
                        {acao.respostaOrigem && <p><span className="font-bold text-[var(--text-muted)]">Resposta:</span> {acao.respostaOrigem}</p>}
                        {acao.multaEstimada && <p><span className="font-bold text-[var(--text-muted)]">Multa / Impacto potencial:</span> {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(acao.multaEstimada)}</p>}
                        {acao.chanceIncidente && <p><span className="font-bold text-[var(--text-muted)]">Chance de incidente:</span> {acao.chanceIncidente}%</p>}
                     </div>
               ...
               <>
                 <div className="grid grid-cols-2 gap-y-4 text-[13px]">
                    <div className="flex items-center gap-2 text-[var(--text-secondary)]">
                       ...
                       <span className="text-[var(--text-muted)] w-24">Responsável</span>
                       <div className="flex items-center gap-1.5 ml-auto text-[var(--text-primary)]">
                    ...
                    <div className="flex items-center gap-2 text-[var(--text-secondary)] col-start-1">
                       <CalendarClock className="w-4 h-4 text-[var(--text-muted)]" />
                       <span className="text-[var(--text-muted)] w-24">Concluída em</span>
                    ...
                    <div className="flex items-center gap-2 text-[var(--text-secondary)] col-span-2">
                       ...
                       <span className="text-[var(--text-muted)] w-24">Risco vinculado</span>
                       <span className="ml-auto text-[var(--text-primary)] truncate max-w-[200px]">{acao.riscoVinculado || 'Esmagamento de membros superiores'}</span>
                    </div>
                    <div className="flex items-center gap-2 text-[var(--text-secondary)] col-span-2">
                       <ClipboardCheck className="w-4 h-4 text-[var(--text-muted)]" />
                       <span className="text-[var(--text-muted)] w-32">Inspeção vinculada</span>
                       <span className="ml-auto text-[var(--text-primary)]">Auditoria Interna - 28/04/</span>
                    </div>
                    <div className="flex items-center gap-2 text-[var(--text-secondary)] col-span-2">
                       <BookOpen className="w-4 h-4 text-[var(--text-muted)]" />
                       <span className="text-[var(--text-muted)] w-24">NR aplicável</span>
                       <span className="ml-auto text-[var(--text-primary)]">NR-12</span>
                    </div>
                 </div>

                 <div className="space-y-2">
                    <h3 className="text-[13px] font-medium text-[var(--text-primary)] border-b border-[var(--border)] pb-2">O que foi feito / Comentários</h3>
                    <div className="text-[13px] text-[var(--text-muted)] leading-relaxed pt-1 space-y-2">
                       {acao.comentarios && acao.comentarios.length > 0 ? (
                         acao.comentarios.map((comentario, index) => (
                           <div key={index} className="bg-[var(--bg-active-group)] p-3 rounded-lg border border-[var(--border)]">
                             {comentario}
                           </div>
                         ))
                       ) : (
                         <div className="italic text-[var(--text-muted)]">Nenhum comentário registrado ainda.</div>
                       )}
                    </div>
                 </div>

                 <div className="space-y-3">
                    <h3 className="text-[13px] font-medium text-[var(--text-primary)] border-b border-[var(--border)] pb-2">Evidências e anexos</h3>
                    <div className="flex gap-3 overflow-x-auto no-scrollbar pt-1">
                       <div className="w-24 shrink-0 rounded-lg border border-[var(--border)] overflow-hidden group cursor-pointer relative">
                          ...
                          <div className="absolute inset-0 bg-[var(--bg-primary)] opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                             <ExternalLink className="w-4 h-4 text-[var(--text-primary)]" />
                          </div>
                       </div>
                       <div className="w-24 shrink-0 rounded-lg border border-[var(--border)] overflow-hidden group cursor-pointer relative">
                 ...
                 <div className="space-y-3">
                    <h3 className="text-[13px] font-medium text-[var(--text-primary)] border-b border-[var(--border)] pb-2">Impacto desta ação</h3>
            ...
            {isAndamento && (
               <div className="space-y-4">
                  <h3 className="text-[13px] font-medium text-[var(--text-primary)]">Checklist de execução</h3>
                  <div className="space-y-3">
                     <div className="flex items-start justify-between text-[13px] gap-3">
                        <div className="flex items-start gap-2.5">
                           <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" />
                           <span className="text-[var(--text-secondary)]">Identificar EPIs vencidos</span>
                        </div>
                        <span className="text-[var(--text-muted)] text-[12px]">30/05</span>
                     </div>
                     <div className="flex items-start justify-between text-[13px] gap-3">
                        <div className="flex items-start gap-2.5">
                           <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" />
                           <span className="text-[var(--text-secondary)]">Aprovar aquisição</span>
                        </div>
                        <span className="text-[var(--text-muted)] text-[12px]">31/05</span>
                     </div>
                     <div className="flex items-start justify-between text-[13px] gap-3">
                        <div className="flex items-start gap-2.5">
                           <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" />
                           <span className="text-[var(--text-secondary)]">Receber novos EPIs</span>
                        </div>
                        <span className="text-[var(--text-muted)] text-[12px]">01/06</span>
                     </div>
                     <div className="flex items-start justify-between text-[13px] gap-3">
                        <div className="flex items-start gap-2.5">
                           <Circle className="w-4 h-4 text-blue-500 shrink-0 mt-0.5 border-2 border-blue-500 rounded-full bg-blue-500/20" />
                           <span className="text-[var(--text-secondary)]">Substituir e registrar entrega</span>
                        </div>
                        <span className="text-[var(--text-muted)] text-[12px]">Em andamento</span>
                     </div>
                     <div className="flex items-start justify-between text-[13px] gap-3">
                        <div className="flex items-start gap-2.5">
                           <Circle className="w-4 h-4 text-[var(--text-muted)] shrink-0 mt-0.5" />
                           <span className="text-[var(--text-muted)]">Treinar equipe sobre uso</span>
                        </div>
                        <span className="text-[var(--text-muted)] text-[12px]">Pendente</span>
                     </div>
                  </div>
               </div>
            )}
            ...
            {!isConcluida && (
            <div>
               <h3 className="text-[13px] font-medium text-[var(--text-primary)] mb-3">Próximos passos</h3>
               {isAndamento ? (
                  <ul className="list-disc list-inside text-[13px] text-[var(--text-muted)] space-y-2 marker:text-[var(--text-secondary)]">
                     <li>Concluir distribuição dos EPIs para a equipe do turno B</li>
                     <li>Registrar entrega e assinatura dos colaboradores</li>
                  </ul>
               ) : (
                  <ol className="list-decimal list-inside text-[13px] text-[var(--text-muted)] space-y-2 marker:text-[var(--text-secondary)]">
            ...
            {/* Metadados */}
            {!isConcluida && (
            <div className="space-y-0 text-[13px] border-t border-[var(--border)] pt-4">
               {(!isAndamento) && (
                  <div className="flex items-center py-3 border-b border-[var(--border)]">
                     <div className="flex items-center gap-2.5 w-1/2 text-[var(--text-muted)]">
                        <Factory className="w-4 h-4" />
                        <span>Setor</span>
                     </div>
                     <div className="w-1/2 text-[var(--text-primary)]">{acao.setor || 'Obras / Estruturas'}</div>
                  </div>
               )}
               ...
               <div className="flex items-center justify-between py-3 border-b border-[var(--border)]">
                  ...
                  <div className="flex items-center gap-2">
                     <span className="text-[var(--text-primary)] text-[13px]">{acao.responsavel}</span>
               ...
               {isAndamento && (
                  <div className="flex items-center justify-between py-3 border-b border-[var(--border)]">
                     ...
                     <div className="text-[var(--text-primary)]">{acao.setor || 'Obras / Estruturas'}</div>
                  </div>
               )}
               ...
               <div className="flex items-center justify-between py-3 border-b border-[var(--border)]">
                  ...
                  <div className="text-[var(--text-primary)] flex items-center gap-2">
                     {acao.riscoVinculado || '-'}
                     {acao.riscoStatus && <span className="text-[10px] bg-[var(--bg-active-group)] px-1.5 py-0.5 rounded">{acao.riscoStatus}</span>}
                  </div>
               </div>
               ...
               {acao.riscoSeveridade && (
                  <div className="flex items-center justify-between py-3 border-b border-[var(--border)]">
                     ...
                     <div className="text-[var(--text-primary)]">{acao.riscoSeveridade}</div>
                  </div>
               )}
               ...
               {acao.perguntaOrigem && (
                  <div className="flex items-start py-3 border-b border-[var(--border)]">
                     <div className="flex items-center gap-2.5 w-1/3 text-[var(--text-muted)] mt-0.5">
                        <ListChecks className="w-4 h-4" />
                        <span>Checklist / Pergunta</span>
                     </div>
                     <div className="w-2/3 text-[var(--text-primary)] leading-snug">{acao.perguntaOrigem}</div>
                  </div>
               )}
               ...
               {acao.respostaOrigem && (
                  <div className="flex items-start py-3 border-b border-[var(--border)]">
                     ...
                     <div className="w-2/3 text-[var(--text-primary)] leading-snug">{acao.respostaOrigem}</div>
                  </div>
               )}
               ...
               {acao.nrRelacionada && acao.nrRelacionada !== 'Não informada' && (
                  <div className="flex items-center justify-between py-3 border-b border-[var(--border)]">
                     ...
                     <div className="text-[var(--text-primary)]">{acao.nrRelacionada}</div>
                  </div>
               )}
               ...
               {isAndamento && (
                  <div className="flex items-center justify-between py-3 border-b border-[var(--border)]">
                     ...
                     <div className="text-[var(--text-primary)]">{acao.iniciadoEm ? new Date(acao.iniciadoEm).toLocaleDateString('pt-BR') : '30/05/2025'}</div>
                  </div>
               )}

               {(!isAndamento) && (
                  <div className="flex items-start py-3 border-b border-[var(--border)]">
                     <div className="flex items-center gap-2.5 w-1/2 text-[var(--text-muted)] mt-0.5">
                        <CalendarClock className="w-4 h-4" />
                        <span>Prazo</span>
                     </div>
                     <div className="w-1/2">
               ...
               {isAndamento && (
                  <div className="flex items-start justify-between py-3 border-b border-[var(--border)]">
                     <div className="flex items-center gap-2.5 text-[var(--text-muted)] mt-0.5">
                        <CalendarClock className="w-4 h-4" />
                        <span>Prazo</span>
                     </div>
               ...
               {(!isAndamento) && (
                  <div className="flex items-start py-3">
                     ...
                     <div className="w-1/2">
                        <div className="text-[var(--text-primary)] font-medium tracking-wide">
                           {acao.multaEstimada ? `R$ ${acao.multaEstimada.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` : 'R$ 25.000,00'}
                        </div>
                        <div className="text-[11px] text-[var(--text-muted)] mt-0.5">{acao.chanceIncidente ? `Chance: ${acao.chanceIncidente}` : 'Interdição e autuação'}</div>
                     </div>
                  </div>
               )}
            </div>
            ...
            {acao.followUp && acao.followUp.ativo && (
               <div className="bg-[var(--bg-secondary)] border border-[var(--border)] rounded-[10px] p-4 space-y-3">
                  ...
                  <div className="space-y-2 text-[12px]">
                     ...
                     <div className="flex justify-between items-center text-[var(--text-muted)] border-t border-[var(--border)] pt-2 mt-2">
                        <span>Mensagem do sistema</span>
                     </div>
                     <div className="bg-[var(--bg-primary)] p-2 rounded text-[var(--text-secondary)] italic">
                  ...
                  <div className="flex gap-2 pt-2 border-t border-[var(--border)]">
                     <button 
                        ...
                        className="flex-1 py-1.5 bg-[var(--bg-active-group)] hover:bg-[var(--bg-active-group)] text-[var(--text-secondary)] rounded text-[11px] font-medium transition-colors"
                     >
            ...
            {(!isAndamento && !isConcluida) && (
               <div className="bg-[var(--bg-secondary)] p-4 rounded-[10px] border border-[var(--border)]">
                  ...
                  <div className="flex justify-between text-[10px]">
                    <span className="text-[var(--text-muted)]">Início esperado: {acao.prazo}</span>
                    <span className="text-[var(--text-muted)]">{acao.status === 'Pendente' ? '0% iniciado' : `${acao.progresso}% concluído`}</span>
                  </div>
               </div>
            )}
          ...
          <div className="p-6 border-t border-[var(--border)] shrink-0 bg-[#0c1018] space-y-2.5">
             ...
             {acao.followUp?.escalado && !isConcluida && (
                 <div className="flex flex-col gap-2.5">
                    <button className="w-full bg-pink-600 hover:bg-pink-700 text-[var(--text-primary)] py-3 rounded-lg font-medium transition-all flex items-center justify-center gap-2 text-[13px]">
                      Registrar resposta ao escalonamento
                    </button>
                    <div className="flex items-center gap-2.5">
                       <button onClick={() => { setProgressoLocal(acao.progresso || 0); setShowUpdateModal(true); }} className="flex-1 bg-[var(--bg-active-group)] hover:bg-[var(--bg-active-group)] text-[var(--text-secondary)] py-3 rounded-lg font-medium transition-colors border border-[var(--border)] flex items-center justify-center gap-2 text-[13px]">Atualizar progresso</button>
                       <button className="flex-1 bg-[var(--bg-active-group)] hover:bg-[var(--bg-active-group)] text-[var(--text-secondary)] py-3 rounded-lg font-medium transition-colors border border-[var(--border)] flex items-center justify-center gap-2 text-[13px]">Reatribuir</button>
                    </div>
                 </div>
             )}
             ...
             {acao.status === 'Pendente' && !acao.followUp?.escalado && (
                <div className="flex flex-col gap-2.5">
                   <button onClick={() => { iniciarAcao(acao.id); onClose(); }} className="w-full bg-[#4f46e5] hover:bg-[#4338ca] text-[var(--text-primary)] py-3 rounded-lg font-medium transition-all flex items-center justify-center gap-2 text-[13px]">
                     <Play className="w-4 h-4 fill-current" /> Iniciar ação
                   </button>
                   <div className="flex items-center gap-2.5">
                      <button onClick={() => { setProgressoLocal(acao.progresso || 0); setShowUpdateModal(true); }} className="flex-1 bg-[var(--bg-active-group)] hover:bg-[var(--bg-active-group)] text-[var(--text-secondary)] py-2.5 rounded-lg text-[13px] font-medium transition-colors border border-[var(--border)]">Registrar atualização</button>
                      <button className="flex-1 bg-[var(--bg-active-group)] hover:bg-[var(--bg-active-group)] text-[var(--text-secondary)] py-2.5 rounded-lg text-[13px] font-medium transition-colors border border-[var(--border)] flex items-center justify-center gap-2"><UserPlus className="w-4 h-4" /> Reatribuir</button>
                   </div>
             ...
             {acao.status === 'Vencida' && !acao.followUp?.escalado && (
                <div className="flex flex-col gap-2.5">
                   <button onClick={() => { iniciarAcao(acao.id); onClose(); }} className="w-full bg-red-600 hover:bg-red-700 text-[var(--text-primary)] py-3 rounded-lg font-medium transition-all flex items-center justify-center gap-2 text-[13px]">
                     <Play className="w-4 h-4 fill-current" /> Iniciar com prioridade
                   </button>
                   <div className="flex items-center gap-2.5">
                      <button className="flex-1 bg-[var(--bg-active-group)] hover:bg-[var(--bg-active-group)] text-[var(--text-secondary)] py-2.5 rounded-lg text-[13px] font-medium transition-colors border border-[var(--border)] flex items-center justify-center gap-2"><UserPlus className="w-4 h-4"/> Reatribuir</button>
                      <button className="flex-1 bg-[var(--bg-active-group)] hover:bg-[var(--bg-active-group)] text-[var(--text-secondary)] py-2.5 rounded-lg text-[13px] font-medium transition-colors border border-[var(--border)]">Registrar justificativa</button>
                   </div>
                   {renderIntegrationButtons()}
                </div>
             )}
             ...
             {isAndamento && !acao.followUp?.escalado && (
                <div className="flex flex-col gap-2.5">
                   {acao.followUp?.precisaFollowUp ? (
                     <button onClick={() => { setProgressoLocal(acao.progresso || 0); setShowUpdateModal(true); }} className="w-full bg-indigo-600 hover:bg-indigo-700 text-[var(--text-primary)] py-3 rounded-lg font-medium transition-all flex items-center justify-center gap-2 text-[13px]">
                       Registrar atualização
                     </button>
                   ) : (
                     <div className="flex items-center gap-2.5">
                   ...
                   <div className="flex items-center gap-2.5">
                      {acao.followUp?.precisaFollowUp && (
                         <>
                           <button onClick={() => { setProgressoLocal(acao.progresso || 0); setShowUpdateModal(true); }} className="flex-1 bg-[var(--bg-active-group)] hover:bg-[var(--bg-active-group)] text-[var(--text-secondary)] py-2.5 rounded-lg text-[13px] font-medium transition-colors border border-[var(--border)]">Atualizar</button>
                           <button onClick={() => setShowConfirmClose(true)} className="flex-1 bg-[var(--bg-active-group)] hover:bg-[var(--bg-active-group)] text-[var(--text-secondary)] py-2.5 rounded-lg text-[13px] font-medium transition-colors border border-[var(--border)]">Concluir</button>
                         </>
                      )}
                      {!acao.followUp?.precisaFollowUp && (
                        <button className="flex-1 bg-[var(--bg-active-group)] hover:bg-[var(--bg-active-group)] text-[var(--text-secondary)] py-2.5 rounded-lg text-[13px] font-medium transition-colors border border-[var(--border)]">Registrar bloqueio</button>
                      )}
                   </div>
                   {renderIntegrationButtons()}
                </div>
             ...
             {isConcluida && (
                <div className="flex flex-col gap-2.5">
                   ...
                   <button onClick={onClose} className="w-full bg-[var(--bg-active-group)] hover:bg-[var(--bg-active-group)] text-[var(--text-primary)] mt-1 py-3 rounded-lg font-medium transition-colors flex items-center justify-center text-[13px]">Fechar painel</button>
                </div>
             )}
          </div>
        </div>
        ...
        {showConfirmClose && (
           ...
           >
              ...
              >
                 ...
                 <h2 className="text-lg font-medium text-[var(--text-primary)] text-center mb-2">Confirmar conclusão da ação</h2>
                 <p className="text-[13px] text-[var(--text-muted)] text-center mb-6">Tem certeza que esta ação foi concluída?</p>
                 
                 <div className="bg-[var(--bg-active-group)] rounded-xl p-4 mb-6 space-y-3">
                    <div className="flex justify-between text-[13px]">
                       <span className="text-[var(--text-muted)]">Ação:</span>
                       <span className="text-[var(--text-primary)] font-medium text-right max-w-[200px] truncate">{acao.titulo}</span>
                    </div>
                    <div className="flex justify-between text-[13px]">
                       <span className="text-[var(--text-muted)]">Responsável:</span>
                       <span className="text-[var(--text-primary)]">{acao.responsavel}</span>
                    </div>
                    <div className="flex justify-between text-[13px]">
                       <span className="text-[var(--text-muted)]">Risco:</span>
                       <span className="text-[var(--text-primary)]">{acao.riscoVinculado || '-'}</span>
                    </div>
                 </div>

                 <div className="mb-6">
                     <label className="text-[13px] text-[var(--text-muted)] block mb-2">
                 ...
                 <div className="flex gap-3">
                    <button 
                       ...
                       className="flex-1 py-2.5 rounded-lg text-[13px] font-medium text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-active-group)] transition-colors"
                    >
                       Cancelar
                    </button>
                    <button 
                       ...
                       className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-[var(--text-primary)] py-2.5 rounded-lg text-[13px] font-medium transition-colors"
                    >
                       Sim, concluir ação
                    </button>
                 </div>
              ...
              >
                 ...
                 <div className="flex gap-3 mt-6">
                    ...
                    <button 
                       ...
                       className="flex-1 bg-purple-600 hover:bg-purple-700 text-[var(--text-primary)] py-2.5 rounded-lg text-[13px] font-medium transition-colors"
                    >
                       Salvar
                    </button>
                 </div>
