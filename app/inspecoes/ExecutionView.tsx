...
export default function ExecutionView({ inspectionId, onClose }: { inspectionId: string, onClose: () => void }) {
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[100] bg-[var(--bg-primary)] flex flex-col overflow-hidden text-[var(--text-primary)]"
    >
      <div className="flex-none h-20 border-b border-[var(--border)] bg-[var(--bg-primary)]/80 backdrop-blur-xl flex items-center px-8 justify-between">
        <div className="flex items-center gap-6">
          <button onClick={handleBack} className="p-2.5 -ml-2 text-[var(--text-muted)] hover:text-[var(--text-primary)] rounded-xl hover:bg-[var(--bg-active-group)] transition-all border border-transparent hover:border-[var(--border)] group">
             ...
             <div className="flex items-center gap-3">
                ...
                <div>
                   <div className="flex items-center gap-2.5">
                      <h1 className="text-xl font-bold text-[var(--text-primary)] tracking-tight uppercase">Auditoria de Segurança</h1>
                      <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black tracking-widest uppercase border border-purple-500/30 text-purple-400 bg-purple-500/10 shadow-[var(--shadow-glow)] animate-pulse">Live</span>
                   </div>
                   <p className="text-[12px] text-[var(--text-muted)] font-medium">Protocolo: #{inspection.id.substring(0, 8)} • {inspection.checklist}</p>
                </div>
             </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
           <div className="hidden lg:flex items-center gap-6 pr-6 border-r border-[var(--border)]">
              <div className="text-right">
                 <p className="text-[10px] text-[var(--text-muted)] uppercase font-black tracking-widest">Responsável</p>
                 <p className="text-sm font-bold text-[var(--text-primary)]">{inspection.responsavel}</p>
              </div>
              <div className="w-10 h-10 rounded-full border border-[var(--border)] overflow-hidden relative shadow-lg">
           ...
           <button onClick={handleBack} className="p-3 text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-all hover:bg-[var(--bg-active-group)] rounded-xl border border-[var(--border)]">
             <X className="w-5 h-5" />
           </button>
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
         <div className="flex-1 flex flex-col overflow-y-auto custom-scrollbar bg-[var(--bg-primary)]">
            <div className="p-6 lg:p-8 max-w-6xl mx-auto w-full space-y-8 pb-32">
               ...
               <div className="relative">
                  <div className="absolute top-8 left-10 right-10 h-[2px] bg-[var(--bg-active-group)] overflow-hidden">
                     <motion.div 
                        initial={{ width: 0 }}
                        animate={{ width: "75%" }}
                        className="h-full bg-gradient-to-r from-purple-600 to-blue-500 shadow-[var(--shadow-glow)]"
                     />
                  </div>
                     ...
                     {[
                        ...
                        <div key={i} className="flex flex-col items-center group">
                           ...
                           <div className="text-center mt-3">
                              <p className={`text-[12px] font-bold ${s.color === 'gray' ? 'text-[var(--text-secondary)]' : 'text-[var(--text-primary)]'}`}>{s.label}</p>
                  ...
                  <div className="bg-[var(--bg-primary)] border border-[var(--border)] p-5 rounded-2xl group hover:border-[var(--border)] hover:bg-[#0d121e] transition-all relative overflow-hidden">
                     ...
                     <div className="flex flex-col gap-3 relative z-10">
                        ...
                        <div>
                           ...
                           <h3 className="text-2xl font-black text-[var(--text-primary)] tracking-tighter mt-1">{Math.round(progresso * 100)}%</h3>
                           <p className="text-xs text-[var(--text-muted)] mt-1 font-medium">{itensRespondidos} de {items.length}</p>
                        </div>
                     </div>
                  </div>
                  <div className="bg-[var(--bg-primary)] border border-[var(--border)] p-5 rounded-2xl group hover:border-[var(--border)] hover:bg-[#0d121e] transition-all relative overflow-hidden">
                     ...
                     <div className="flex flex-col gap-3 relative z-10">
                        ...
                        <div>
                           ...
                           <h3 className="text-2xl font-black text-[var(--text-primary)] tracking-tighter mt-1">{Math.round(conformidade * 100)}%</h3>
                           <p className="text-xs text-[var(--text-muted)] mt-1 font-medium">{conformidade > 0.7 ? 'Seguro' : 'Auditado'}</p>
                        </div>
                     </div>
                  </div>
                  <div className="bg-[var(--bg-primary)] border border-[var(--border)] p-5 rounded-2xl group hover:border-[var(--border)] hover:bg-[#0d121e] transition-all relative overflow-hidden">
                     ...
                     <div className="flex flex-col gap-3 relative z-10">
                        ...
                        <div>
                           ...
                           <h3 className="text-2xl font-black text-[var(--text-primary)] tracking-tighter mt-1">{ncsDetectadasCount}</h3>
                           <p className="text-xs text-[var(--text-muted)] mt-1 font-medium">Riscos Críticos</p>
                        </div>
                     </div>
                  </div>
                  <div className="bg-[var(--bg-primary)] border border-[var(--border)] p-5 rounded-2xl group hover:border-[var(--border)] hover:bg-[#0d121e] transition-all relative overflow-hidden">
                     ...
                     <div className="flex flex-col gap-3 relative z-10">
                        ...
                        <div>
                           ...
                           <h3 className="text-2xl font-black text-[var(--text-primary)] tracking-tighter mt-1">{acoesGeradasCount}</h3>
                           <p className="text-xs text-[var(--text-muted)] mt-1 font-medium">Prioridade Alta</p>
                        </div>
                     </div>
                  </div>
               </div>

               <div className="flex flex-col md:flex-row items-center justify-between gap-4 py-6 border-y border-[var(--border)]">
                  <h2 className="text-2xl font-black text-[var(--text-primary)] tracking-tight flex items-center gap-3">
                     Processo Auditoria
                     <span className="text-[12px] bg-[var(--bg-active-group)] border border-[var(--border)] px-3 py-1 rounded-full text-[var(--text-muted)] font-bold uppercase tracking-widest">{items.length} ITENS</span>
                  </h2>
                  <div className="flex gap-3 w-full md:w-auto">
                     <div className="relative flex-1 md:w-64">
                        ...
                        <input 
                           ...
                           className="w-full bg-[var(--bg-active-group)] border border-[var(--border)] rounded-xl py-3 pl-11 pr-4 text-sm text-[var(--text-primary)] focus:outline-none focus:border-purple-500/50 focus:ring-1 focus:ring-purple-500/50 transition-all"
                        />
                     </div>
                  </div>
               </div>
                              ...
                              {[
                                 ...
                                 <button 
                                    ...
                                    ${item.status === btn.value ? 
                                       ...
                                       'bg-[var(--bg-primary)] border-[var(--border)] text-[var(--text-muted)] hover:border-[var(--border)] hover:text-[var(--text-secondary)]'}`}
                                 >
                                    {btn.label} {item.status === btn.value && btn.icon && <btn.icon className="w-4 h-4" />}
                                 </button>
                              ))}
                              
                              <button className="h-12 w-12 flex items-center justify-center text-[var(--text-muted)] hover:text-purple-400 transition-all hover:bg-purple-500/10 rounded-xl ml-1 active:scale-90 border border-[var(--border)]">
         ...
         <div className="w-[450px] bg-[var(--bg-primary)] border-l border-[var(--border)] shrink-0 flex flex-col shadow-[0_0_50px_rgba(0,0,0,0.5)] relative z-20">
            <div className="p-8 border-b border-[var(--border)] bg-[#0d121e]/80 backdrop-blur-md">
               <div className="flex items-center justify-between">
                  <div className="flex items-center gap-5">
                     ...
                     <div>
                        <h3 className="font-black text-[var(--text-primary)] text-2xl tracking-tighter uppercase">Audit Vision</h3>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-8 space-y-12">
               <div className="grid grid-cols-2 gap-4">
                  <div className="bg-[var(--bg-primary)] p-6 rounded-3xl border border-[var(--border)] relative overflow-hidden group">
                     <div className="absolute top-0 right-0 w-16 h-16 bg-red-500/5 blur-2xl group-hover:bg-red-500/10 transition-colors"></div>
                     <p className="text-xs text-[var(--text-muted)] font-black uppercase tracking-widest mb-3">Riscos de Auditoria</p>
                     <div className="flex items-baseline gap-2 mt-2">
                        ...
                        <span className="text-sm text-[var(--text-muted)] font-bold ml-1">DETECTADOS</span>
                     </div>
                  </div>
                  <div className="bg-[var(--bg-primary)] p-6 rounded-3xl border border-[var(--border)] relative overflow-hidden group">
               ...
               <div className="space-y-6">
                  <h4 className="text-xs font-black text-[var(--text-muted)] uppercase tracking-[0.25em] pl-1">Matriz de Desvios</h4>
                        ...
                        >
                           ...
                           <div className="flex gap-5 relative z-10">
                              ...
                              <div>
                                 ...
                                 <p className="text-xl font-bold text-[var(--text-primary)] mt-1.5 leading-snug tracking-tighter italic">&quot;{nc.text}&quot;</p>
                              </div>
                           </div>
                           ...
                           <div className="grid grid-cols-2 gap-3 relative z-10">
                              ...
                              <div className="bg-[var(--bg-primary)] p-4 rounded-2xl border border-[var(--border)]">
                                 ...
                                 <p className="text-sm font-black uppercase text-[var(--text-primary)]">{nc.multa}</p>
                              </div>
                           </div>
                           ...
                           <div className="p-6 bg-[var(--bg-primary)] rounded-3xl border border-[var(--border)] text-[14px] leading-relaxed relative z-10 group-hover:bg-red-500/[0.03] transition-colors">
                              <div className="flex items-start gap-4">
                                 <ClipboardCheck className="w-5 h-5 text-purple-400 shrink-0 mt-0.5" />
                                 <div className="space-y-2">
                                    <p className="text-xs text-[var(--text-muted)] font-black uppercase tracking-widest">Ação Corretiva</p>
                                    <p className="text-[var(--text-secondary)] font-medium italic">&quot;{nc.acaoSugerida}&quot;</p>
                                 </div>
                              </div>
                           </div>
                        </motion.div>
                  ...
                  {risks.length === 0 && (
                     <div className="py-24 flex flex-col items-center justify-center text-center px-12 space-y-6 opacity-40">
                        ...
                        <div className="space-y-1">
                           <h4 className="text-lg text-[var(--text-muted)] font-black uppercase tracking-widest">Protocolo Zero Desvio</h4>
                           <p className="text-sm text-[var(--text-secondary)] leading-relaxed font-medium max-w-xs mx-auto">O radar de segurança não detectou inconformidades críticas no momento.</p>
                        </div>
                     </div>
                  )}
                              ...
                              <div key={acao.id} className="bg-[var(--bg-secondary)]/80 border border-[var(--border)] rounded-2xl p-5 flex items-start gap-4">
                                 ...
                                 <div className="flex-1 min-w-0">
                                    <div className="flex items-start justify-between gap-4 mb-2">
                                       <h5 className="text-sm font-bold text-[var(--text-primary)] leading-snug">{acao.titulo}</h5>
            ...
            <div className="p-8 border-t border-[var(--border)] bg-[var(--bg-primary)] space-y-4 shadow-[0_-20px_50px_rgba(0,0,0,0.5)] relative z-30">
               <div className="flex gap-3">
                  <button 
                     onClick={handleSalvar}
                     disabled={isSaving}
                     className="flex-1 py-5 rounded-3xl border border-[var(--border)] text-[var(--text-muted)] font-black text-[12px] uppercase tracking-widest hover:bg-[var(--bg-active-group)] transition-all active:scale-95 flex items-center justify-center gap-3 disabled:opacity-50"
                  >
                     {isSaving ? 'Salvando...' : 'Salvar Respostas'}
                  </button>
                  <button 
                     onClick={handleConfirmarConclusao}
                     className="flex-[2] py-5 rounded-3xl border border-purple-500/50 text-[var(--text-primary)] font-black text-[12px] uppercase tracking-[0.2em] bg-gradient-to-br from-purple-600 to-indigo-700 hover:from-purple-500 hover:to-indigo-600 transition-all shadow-[0_15px_40px_rgba(124,58,237,0.4)] flex items-center justify-center gap-3 active:scale-95 group"
                  >
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="flex flex-col gap-3">
                <button 
                  onClick={() => setConfirmExitOpen(false)}
                  className="w-full py-4 rounded-2xl bg-[var(--bg-active-group)] border border-[var(--border)] text-[var(--text-primary)] font-bold hover:bg-[var(--bg-active-group)] transition-colors"
                >
        ...
        {confirmCompleteOpen && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="w-full bg-[var(--bg-primary)] rounded-3xl p-6 mb-10 text-left space-y-4 border border-[var(--border)]">
                <div className="flex justify-between items-center pb-4 border-b border-[var(--border)]">
                  <span className="text-[var(--text-muted)] font-medium">Itens Respondidos</span>
                  <span className="text-[var(--text-primary)] font-bold">{itensRespondidos} de {items.length}</span>
                </div>
                <div className="flex justify-between items-center pb-4 border-b border-[var(--border)]">
              ...
              <div className="flex gap-4 w-full">
                <button 
                  onClick={() => setConfirmCompleteOpen(false)}
                  className="flex-1 py-5 rounded-2xl bg-[var(--bg-active-group)] border border-[var(--border)] text-[var(--text-primary)] font-bold hover:bg-[var(--bg-active-group)] transition-colors"
                >
                  Cancelar
                </button>
                <button 
                  onClick={executeConcluir}
                  className="flex-[1.5] py-5 rounded-2xl bg-purple-600 text-[var(--text-primary)] font-black uppercase tracking-widest hover:bg-purple-500 transition-colors shadow-lg shadow-purple-500/20"
                >
                  Confirmar Conclusão
                </button>
              </div>
