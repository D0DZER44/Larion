...
function getPrazoClass(prazoStr: string, status: string) {
  if (!prazoStr || typeof prazoStr !== 'string') return 'text-[var(--text-secondary)]';
  ...
  const now = new Date();
  if (isNaN(d.getTime())) return 'text-[var(--text-secondary)]';
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-[var(--bg-secondary)] border border-[var(--border)] p-5 rounded-[12px] flex flex-col justify-between">
           <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="text-[15px] font-medium text-[var(--text-secondary)]">Pendentes totais</h3>
           </div>
        ...
        <div className="bg-[#1e1318] border border-red-500/20 p-5 rounded-[12px] flex flex-col justify-between">
           <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="text-[15px] font-medium text-[var(--text-secondary)]">Críticas pendentes</h3>
           </div>
        ...
        <div className="bg-[#1a1512] border border-orange-500/20 p-5 rounded-[12px] flex flex-col justify-between">
           <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="text-[15px] font-medium text-[var(--text-secondary)]">Vencem hoje</h3>
           </div>
        ...
        <div className="bg-[var(--bg-secondary)] border border-blue-500/20 p-5 rounded-[12px] flex flex-col justify-between">
           ...
           <div className="text-[32px] leading-tight font-bold text-[var(--text-primary)] mt-1">{stats.estaSemanaCount}</div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-6 bg-[var(--bg-secondary)] border border-[var(--border)] p-4 rounded-[12px]">
            ...
            <div className="flex gap-2">
               {['Todos', 'Crítica', 'Alta', 'Média', 'Baixa', 'Exige atenção'].map(p => (
                  <button 
                     ...
                     className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${filterPriority === p ? 'bg-[var(--bg-active-group)] text-[var(--text-primary)]' : 'text-[var(--text-muted)] hover:bg-[var(--bg-active-group)]'}`}
                  >
                     {p}
                  </button>
               ))}
         ...
         <div className="flex items-center gap-3">
            <span className="text-[13px] font-medium text-[var(--text-muted)]">Prazo:</span>
            <div className="flex gap-2">
               {['Todos', 'Hoje', 'Esta semana'].map(p => (
                  <button 
                     key={p} 
                     onClick={() => setFilterPrazo(p)}
                     className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${filterPrazo === p ? 'bg-[var(--bg-active-group)] text-[var(--text-primary)]' : 'text-[var(--text-muted)] hover:bg-[var(--bg-active-group)]'}`}
                  >
                     {p}
                  </button>
               ))}
                 ...
                 return (
                    <tr key={acao.id} onClick={() => onOpen(acao)} className="hover:bg-[var(--bg-active-group)] transition-colors cursor-pointer group">
                       ...
                       <td className="px-5 py-4 w-48">
                          <span className="text-[13px] text-[var(--text-secondary)] truncate max-w-[150px] inline-block">{acao.riscoVinculado || '-'}</span>
                       </td>
                       <td className="px-5 py-4 w-36">
                          <span className="text-[13px] text-[var(--text-secondary)] truncate max-w-[120px] inline-block">{acao.setor || '-'}</span>
                       </td>
