...
export default function CentralPage() {
  ...
  const { riscos = [], inspecoes = [], acoes = [], checklists = [] } = storeState;

  const [selectedDrawerItem, setSelectedDrawerItem] = useState<any>(null);
  const centralPageModel = useMemo(() => buildTargetCentralPageModel(storeState), [storeState]);
  ...
  const listRiscos = topRisksByChance.length > 0 ? openRisks.filter(r => getRiskSeverityLevel(r) === 'Crítico').slice(0,5) : [];
        ...
        <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6 shrink-0">
          ...
          <div className="flex flex-col gap-6 shrink-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 shrink-0">
              {centralPageModel.topCards.map((card, i) => {
                if (!card) return null;
                return (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 shrink-0">
                ...
                <div className="flex-1 flex flex-col sm:flex-row items-center justify-center gap-8">
                  <div className="w-[180px] h-[180px] relative">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie
                          data={centralPageModel.riskPieData}
                          cx="50%"
                          cy="50%"
                          innerRadius={60}
                          outerRadius={80}
                          ...
                          stroke="none"
                        >
                          {centralPageModel.riskPieData.map((entry, index) => (
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center font-sans mt-1">
                      <span className="text-3xl font-bold text-white leading-none">{centralPageModel.openRisks.length || 128}</span>
                      <span className="text-[12px] text-gray-400 font-medium mt-1">Total</span>
                    </div>
                  </div>
                  <div className="flex flex-col gap-3 font-sans">
                    {centralPageModel.riskPieData.length > 0 ? centralPageModel.riskPieData.map((entry) => (
                      <div key={entry.name} className="flex items-center gap-3">
                        ...
                        <span className="text-[13px] font-bold text-white">{entry.value} <span className="font-normal text-gray-500">({Math.round((entry.value/Math.max(1, centralPageModel.openRisks.length))*100)}%)</span></span>
                      </div>
                    )) : (
              ...
              <div className="bg-[#0e1322] border border-white/10 rounded-xl p-6 flex flex-col">
                ...
                <div className="flex-1 min-h-[220px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={centralPageModel.riskBarData} margin={{ top: 20, right: 10, left: -20, bottom: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.03)" vertical={false} />
                      <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#6b7280' }} dy={10} />
                      <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#6b7280' }} />
                      <Tooltip content={<CustomTooltip />} cursor={{fill: 'rgba(255,255,255,0.02)'}} />
                      <Bar dataKey="Total" fill="#7c3aed" radius={[2, 2, 0, 0]} barSize={32}>
                        {centralPageModel.riskBarData.map((entry, index) => (
              ...
              <div className="bg-[#0e1322] border border-white/10 rounded-xl p-6 flex flex-col">
                ...
                <div className="flex-1 min-h-[220px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart layout="vertical" data={centralPageModel.nrBarData} margin={{ top: 0, right: 40, left: 0, bottom: 0 }}>
                      ...
                      <Bar dataKey="Total" fill="#eab308" radius={[0, 2, 2, 0]} barSize={16}>
                          {centralPageModel.nrBarData.map((entry, index) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 shrink-0 mt-2">
               ...
               <div className="bg-[#0e1322] border border-purple-500/20 rounded-xl p-6 flex flex-col shadow-[0_0_30px_rgba(124,58,237,0.03)] font-sans relative overflow-hidden">
                  ...
                  <div className="flex-1 space-y-5 relative z-10">
                     ...
                     <div className="flex gap-4">
                        ...
                        <div>
                           <p className="text-[13px] font-medium text-gray-200">NR-12 concentra {Math.round(((centralPageModel.nrBarData[0]?.Total || 412000) / (centralPageModel.multaEmAberto||1.248e6)) * 100) || 33}% da multa estimada total.</p>
                     ...
                     <div className="flex gap-4">
                        ...
                        <div>
                           <p className="text-[13px] font-medium text-gray-200">Chance média de incidente em nível {(centralPageModel.avgChance||32) < 40 ? 'moderado' : 'alto'}.</p>
               ...
               <div className="bg-[#0e1322] border border-white/10 rounded-xl p-6 flex flex-col font-sans">
                  ...
                  <div className="flex-1 flex flex-col justify-between py-2 space-y-5">
                     {centralPageModel.renderTopRisksChance.map((r, i) => (
            ...
            <div className="mt-6 shrink-0 font-sans">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                 {centralPageModel.gridKPIs.map(card => {
                    if (!card) return null;
                    return (
                 ...
                 <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                       ...
                       <tbody className="divide-y divide-white/5">
                          {centralPageModel.latestInspections.length === 0 ? (
                            <tr><td colSpan={4} className="p-8 text-center text-[13px] text-gray-500">Nenhuma inspeção recente.</td></tr>
                          ) : centralPageModel.latestInspections.map((insp:any, i:number) => (
                     ...
                     <>
                        ...
                        <div className="space-y-4">
                           <div className="p-4 bg-white/5 rounded-xl border border-white/5">
                              ...
                              <div className="text-xl font-bold text-yellow-500">{formatCurrency(calcularMultaEstimada(selectedDrawerItem.data).multaEstimada)}</div>
                           </div>
                           <div className="p-4 bg-white/5 rounded-xl border border-white/5">
