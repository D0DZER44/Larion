...
import { 
  ...
  LogOut, Moon, Sun, Workflow, ChevronRight
} from 'lucide-react';
...
const navGroups = [
  {
    items: [
      ...
      { 
        name: 'Operação', 
        href: '#', // We will handle click manually or use the first subitem
        icon: Workflow,
        subItems: [
    ...
    <aside className="w-[260px] flex-shrink-0 flex flex-col h-screen bg-[#0b0f19] border-r border-white/5 top-0 sticky print:hidden">
      ...
      <div className="flex-1 overflow-y-auto px-3 py-4 space-y-1 scrollbar-none">
        {navGroups.flatMap(g => g.items as any[]).map((item) => {
          ...
          const isActive = pathname === item.href || itemAny.isActive || (hasSubItems && pathname.startsWith('/operacao') && item.name === 'Operação');
          
          return (
            <div key={item.name} className="flex flex-col">
              <Link
                href={hasSubItems ? '/operacao/inspecoes' : item.href}
                prefetch={true}
                onClick={(e) => {
                  if (hasSubItems) {
                    e.preventDefault();
              ...
              >
                <div className="flex items-center gap-3 w-full">
                  <item.icon className="w-[18px] h-[18px] shrink-0" style={{ color: isActive && !hasSubItems ? '#c084fc' : (itemAny.color ? 'var(--tw-colors-purple-400)' : 'currentColor') }} />
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {hasSubItems && (
                    <ChevronRight className={`w-4 h-4 transition-transform duration-200 ${isExpanded ? 'rotate-90 text-white' : 'text-gray-500'}`} />
                  )}
                </div>
              </Link>
              
              {hasSubItems && isExpanded && (
                <div className="mt-1 space-y-1 ml-4 pl-3 border-l border-white/10">
                  {itemAny.subItems.map((sub: any) => {
                    ...
                    return (
                      <Link
                        ...
                        href={sub.href}
                        onClick={onClose}
                        className={`flex items-center gap-3 px-3 py-2 rounded-lg text-[12px] font-medium transition-all duration-200 ${
                          isSubActive 
                            ? 'bg-purple-500/10 text-purple-400 font-bold border border-purple-500/10' 
                            : 'text-gray-400 hover:text-gray-200 hover:bg-white/5 border border-transparent'
                        }`}
                      >
                         <sub.icon className="w-[16px] h-[16px] shrink-0" style={{ color: isSubActive ? '#c084fc' : 'currentColor' }} />
                         {sub.name}
                      </Link>
                    );
                  })}
