// LEGACY ENGINE LAYER
// Este arquivo permanece para compatibilidade com o fluxo antigo
// `Inspecao -> Risco -> Acao`.
// Toda estrutura nova do motor deve nascer em `src/`.
//
// lib/engines.ts

import { useAppStore, AppState, nrToPackage } from './store';
import { FineEngine } from './fineEngine';
import { enrichChecklistQuestion } from './normativeChecklists';
...
export const AutomationEngine = {
  /**
   * Cada item do checklist carrega os 8 campos do motor de risco
   * (NR, pacote, criticidade, ação sugerida, exige evidência, gera risco,
   * prazo, bloqueante). Esta função usa esses campos como fonte primária:
   * resposta "Não/Parcialmente" gera Risco + Ação automáticos com herança
   * dos atributos do próprio item; `geraRisco === false` evita o disparo.
   * As regras de risco do segmento (state.riskRules) ficam como fallback.
   */
  processInspection(inspectionId: string) {
    ...
    const criteria = ['não', 'nao', 'não conforme', 'nao conforme', 'irregular', 'falha', 'ausente', 'crítico', 'critico'];
    const nonConformingItems = inspection.items.filter((item: any) => {
      const s = item.status;
      if (s === false) return true;
      if (typeof s === 'string') {
         return criteria.includes(s.trim().toLowerCase());
      }
      return false;
    });

    nonConformingItems.forEach((rawItem: any) => {
      const item = enrichChecklistQuestion(rawItem, { pacote: undefined, nr: undefined });
      if (item.geraRisco === false) return;
      ...
      const nr = item.nrRelacionada || matchingRule?.nrRelacionada || item.nr;
      const severity = item.criticidade || matchingRule?.criticidade || 'Médio';
      const pacote = item.pacote || matchingRule?.pacote;
      const prazoHoras = typeof item.prazoPadraoHoras === 'number'
        ? item.prazoPadraoHoras
        : (matchingRule?.prazoPadraoHoras || 72);
      const exigeEvidencia = item.exigeEvidencia !== undefined
        ? !!item.exigeEvidencia
        : !!matchingRule?.exigeEvidencia;

      let titulo = matchingRule?.titulo || item.text || (nr ? `${inspection.checklist || 'Checklist'} - ${nr}` : null) || `Risco operacional em ${inspection.ondeUsar || inspection.setor || 'setor não definido'}`;
      if (!titulo || titulo === 'Sem título' || titulo === 'Risco não especificado') {
         titulo = `Risco operacional em ${inspection.ondeUsar || inspection.setor || 'setor não definido'}`;
      }

      const targetRegraId = item.regraId || matchingRule?.id;
      const targetChecklistItemId = item.id;
      const targetNr = nr;

      const existingRisk = state.riscos.find(r => {
        const isSameInsp = (r.inspectionId === inspectionId || r.inspection_id === inspectionId);
        if (!isSameInsp) return false;
        
        if (targetRegraId && r.regraId === targetRegraId) return true;
        
        const rChecklistItemId = r.checklistItemId || r.checklist_item_id;
        const rNr = r.nr || r.nrRelacionada;
        if (!targetRegraId && rChecklistItemId === targetChecklistItemId && rNr === targetNr) return true;
      ...
      if (!existingRisk) {
        const riskPayload: any = {
          id: `risk-${inspectionId}-${item.id}-${crypto.randomUUID().substring(0,6)}`,
          titulo,
          descricao: `Desvio identificado na inspeção: ${item.text}`,
          origem: "inspecao",
          inspectionId: inspectionId,
          checklistId: inspection.checklistId || inspection.id,
          checklistItemId: targetChecklistItemId,
          setor: inspection.ondeUsar || inspection.setor || 'Geral',
        ...
        if (targetRegraId) riskPayload.regraId = targetRegraId;
        if (nr) riskPayload.nr = nr;
        if (pacote) riskPayload.pacote = pacote;
        if (inspection.tipoInspecao || inspection.atividade) riskPayload.atividade = inspection.tipoInspecao || inspection.atividade;
        if (prazoHoras) riskPayload.prazo = `Até ${prazoHoras}h`;
        if (inspection.responsavel) riskPayload.responsavel = inspection.responsavel;
